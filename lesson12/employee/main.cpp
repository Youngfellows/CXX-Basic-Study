...
int main()
{
    ...
    Manager *manager = new Manager(100,"黄大仙",0.0);//实例化一个子类的指针变量 == 管理员

    employee = manager;//基类的指针变量指向子类对象
    manager->setSalary(12800);
    salary = employee->earnings();
    std::cout << "管理人的工资: " << salary << std::endl;
    employee->show();

    HourlyWorker *hourlyWorker = new HourlyWorker(101,"小莉莉",0,0);//实例化一个子类的指针变量 == 小时工
    ...
    PieceWorker *pieceWorker = new PieceWorker(102,"小龙女",0.0,0);//实例化一个子类的指针变量 == 计件工
