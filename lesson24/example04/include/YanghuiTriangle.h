...
{
    private:
        int number[M][M];//定义一个M * M的二维数组
        int **pNum;//指向指针的指针变量,来指向二维数组
        int *pNum1[M];//使用指针数组,来指向二维数组元素
        int (*pNum2)[M];//使用指向一维数组的指针变量来指向二维数组

    public:
        YanghuiTriangle();//构造函数
        ~YanghuiTriangle();//析构函数
        int **getEle();//获取二维数组元素,返回的是一个指向指针的指针
        int (*getElement())[M];//获取二维数组元素,返回的是一个指向一维数组元素的指针变量
        int ** parse(int **pNum);//解析杨辉三角
        int (*parse(int (*pNum)[M]))[M];//获取杨辉三角,参数是指向一维数组的指针变量,返回值也是指向一维数组的指针变量
        int **parse2(int (*pNum)[M]);//解析杨辉三角,参数是指向一维数组的指针
        int (*parse2(int **pNum))[M];//获取杨辉三角,参数是指向指针的指针,返回值是指向一维数组的指针
        void display();//显示杨辉三角
        ...
        void display(int (*pNum)[M]);//显示杨辉三角,参数是指向一维数组的指针变量
