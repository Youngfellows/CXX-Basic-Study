...
ProducerConsumer::ProducerConsumer(AudioCallback callback)
{
    ...
    this->mCacheSize = 5;
    this->mProducerNumber = 2;
    this->mConsumerNumber = 4;
...
void ProducerConsumer::productAudio()
{
    while (mThreadRun)
    {
        ...
        //丢掉超过缓冲区大小的一部分,这种方式会导致数据丢失,还有可能导致消费者线程一直获取不到锁，导致一直无法消费数据
        if (this->mCacheList->size() > this->mCacheSize)
        {
            // Audio audio = std::move(this->mCacheList->front()); //右值取出数据
            // this->mCacheList->pop_front();                      //删除队列中头部的数据
            // cout << "drop audio,id:" << audio.getId() << ",data:" << audio.getData() << endl;
            cout << "productAudio():: cache is full ..." << endl;
        }
        else
        {
    ...
    while (mThreadRun)
    {
        ...
        if (!this->mCacheList->empty())
        {
            ...
            if (this->mCallback != nullptr)
            {
                ...
                mCallback(audio); //回调数据
            }
        }
        else
        {
            cout << "consumeAudio():: caches is empty ..." << endl;
        }
